...
class AnimatedSignCharacter extends StatefulWidget {
  ...
  const AnimatedSignCharacter({super.key, required this.word, this.onComplete});
...
class _AnimatedSignCharacterState extends State<AnimatedSignCharacter>
  ...
  void initState() {
    ...
    _controller = AnimationController(
      duration: Duration(
        milliseconds: (_currentGesture.duration * 1000).round(),
      ),
      vsync: this,
    );
    ...
    _controller.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        if (_currentPositionIndex <
            _currentGesture.rightHandPositions.length - 1) {
          setState(() {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final currentPosition =
            _currentGesture.rightHandPositions[_currentPositionIndex];
        final currentLeftPosition =
            _currentGesture.leftHandPositions[0]; // Usually static
...
class CharacterPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Draw body
    canvas.drawLine(center.translate(0, 40), center.translate(0, 120), paint);
